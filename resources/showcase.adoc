...
The remark is a sentence about the revision rather than another value belonging
to it, so it is set apart below the labels. A revision that is a date with no
number is labelled `Date`, because a date is not a version.
