...
Almost everything on this page survives the crossing: headings, lists, tables
with their spans and footers, admonitions, quotes and verses, listings with
their callouts, images, footnotes and the page breaks below. The diagrams are
drawn into it as vector graphics rather than pictures of diagrams, so they are
as sharp printed as they are here.

Four things are deliberately left short:

Equations are shown as the notation they were written in:: Typst has a
mathematics mode, but its syntax is neither LaTeX's nor AsciiMath's, so an
equation cannot simply be handed over.
