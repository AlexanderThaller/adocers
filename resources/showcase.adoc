...
ordinary pair of columns. The appendix below is one.
...
[glossary]
block:: A paragraph, a list, a table, a diagram — anything with a shape of its
own. AsciiDoc is made of them.
fragment:: A rendering of the body alone, with no page around it, for embedding
in one somebody else wrote.
oracle:: Asciidoctor, against whose output this renderer's is checked. When the
two disagree, it is right unless there is a reason in writing.
