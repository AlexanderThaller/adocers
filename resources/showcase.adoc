...
With `:stem:` set, a `[stem]` block is an equation. It is typeset in the browser
by MathJax, which is vendored and compiled in the way the drawing module is, so
the page reaches no further than the machine that rendered it.
...
`:stem:` was set to. The two are delimited differently on the way to the page —
`\$…\$` for AsciiMath, `\[…\]` for LaTeX — which is how the typesetter knows
which reading to apply.
...
`--no-math` leaves an equation as the notation it was written in, and
`--mathjax-url` loads MathJax from somewhere else instead. A build without the
`math` feature has no copy to load and behaves as though `--no-math` were always
set — which is the whole cost of the feature, 2.2{nbsp}MB of JavaScript.
...
and is redrawn if you change it. A diagram is drawn to the width of the column
and no wider, so it is there to be read rather than to be scrolled at, and it
...
A pie chart needs no arrows at all:
...
And a git graph draws branches and merges from the commands that made them:
