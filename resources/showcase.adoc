...
With `:stem:` set, a `[stem]` block is delimited for a maths renderer rather
than typeset here: `\$…\$` for AsciiMath and `\[…\]` for LaTeX. Nothing on the
page draws it, so what follows is the delimiters and the source — load MathJax
or KaTeX into the page and they become an equation.
