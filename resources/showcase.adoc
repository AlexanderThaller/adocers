...
`date`, `keywords`, `category`, `edition`, `organization` and `copyright`, and
the ones a code review sets about itself and its findings: `fixed-point`,
`head`, `diff`, `spec`, `standards`, `findings`, `axis`, `severity`, `kind`,
`where` and `source`.
...
A list of tags, keywords or standards is a set of separate things that happens
to be written with commas, so each is shown as its own mark rather than run
...
An icon or a mark can sit in a line of text — image:showcase-heart.svg[A
heart,16,16] — with the same attributes a block image takes, and it is placed
in the line rather than named, in a PDF as well as on a page.
...
Sections are not the only things with addresses. An anchor before a term in a
list gives that term one, so a word can point at the entry that explains it: a
<<block,block>> is one thing and a <<fragment>> is another, both of them
entries in the <<glossary>> at the end of this page. A reference with no text
of its own takes the term it points at.
...
Any other style is a name rather than a shape: it becomes a class on the list
for a stylesheet to find, and the terms are left for that style to set. That is
how a glossary is written — `[glossary]` on the section, and again on the list
inside it, which is what marks the entries as glossary entries rather than as an
ordinary pair of columns. The <<glossary>> at the end of this page is one.
...
Hover over any of these blocks and a button appears in its corner that puts the
code on the clipboard — callout marks left behind, since they are the page's
annotations rather than the reader's code. It is the one script a rendered page
carries, and `--no-copy` leaves it out.
...
this page has a use for. A diagram with a title is captioned and numbered as a
figure, in one sequence with the pictures further down — `:figure-caption:`
renames the label for a document that is not written in English.
...
.A reader, the server and the file system
[mermaid]
----
sequenceDiagram
...
.The same exchange, with its branches and its frames
[mermaid]
----
sequenceDiagram
...
.A branch, a few commits and a merge
[mermaid]
----
gitGraph
    commit id: "parser"
...
An image with a title is captioned and numbered as a figure, in the sequence
the diagrams above share.
...
derived from the file name when you do not give any. `width` and `height` are
in pixels, and a picture wider than the column is brought down to fit rather
than left to run off it.

.A badge, drawn with a gradient, a curve and text
image::showcase-badge.svg[A badge reading adocers,320,120]

An SVG is placed as itself rather than as a picture of itself, so it is as sharp
printed as it is on screen. The badge above leans on the parts of the format a
renderer most often gets wrong — a gradient, a curve, and text that has to find
a font of its own.

.A heart, at a quarter of the width it was drawn at
image::showcase-heart.svg[A heart,86,79]

That one is https://commons.wikimedia.org/wiki/File:A_perfect_SVG_heart.svg[a
Wikimedia Commons file] by `pd4u`, dedicated to the public domain; the other two
were drawn for this page.
...
is run. This page takes about 290 ms, against 70 ms as HTML.
...
Equations are typeset in both notations. Typst's mathematics syntax is neither
LaTeX's nor AsciiMath's — it only looks like the second — so an equation is
translated rather than handed over: LaTeX by
https://crates.io/crates/mitex[mitex], AsciiMath by walking the same tree the
`MathML` above is built from. If some equation's translation will not typeset,
the whole document is rendered again with every equation shown as its source,
rather than not rendered at all.
...
|`--no-copy`
|Leave verbatim blocks without their copy button, and the page without script.
...
Each term below carries an anchor, and the section carries one of its own; what
that is for is shown in <<references>>.

[glossary]
[[block]]block:: A paragraph, a list, a table, a diagram — anything with a shape
of its own. AsciiDoc is made of them.
[[fragment]]fragment:: A rendering of the body alone, with no page around it,
for embedding in one somebody else wrote.
[[oracle]]oracle:: Asciidoctor, against whose output this renderer's is checked.
When the two disagree, it is right unless there is a reason in writing.
