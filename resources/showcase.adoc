...
ordinary pair of columns. The <<glossary>> at the end of this page is one, and
its entries can be pointed at by name: a <<block,block>> is one thing and a
<<fragment>> is another.
...
An anchor before a term gives it an address, so a word in the text can point at
the entry that explains it. The section carries one too, which is what
<<glossary>> in the sentence above pointed at.
