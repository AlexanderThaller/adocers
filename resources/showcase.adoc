...
A `----` block is verbatim. Adding a language emits the class and `data-lang`
attribute that a client-side highlighter looks for — this renderer sets up the
hooks but does not highlight anything itself.
...
<2> They render as an ordered list beneath the block.
